...
class DashboardView(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(self, text="Lokalny agent ochrony z ClamAV i powiadomieniami pulpitu.", bg=BG, fg="#d7ddff", font=("Segoe UI", 13)).pack(anchor="w", padx=24)
    ...
    def refresh(self):
        ...
        self.cards["Stan"][1].config(text="ACTIVE = agent działa automatycznie po starcie GUI.")
        ...
        self.cards["ClamAV"][0].config(text="READY" if state["clamav_ready"] else "OFFLINE")
...
class MusgGuardApp(tk.Tk):
    ...
    def update_signatures(self):
        ...
        messagebox.showinfo("Musg Guard", "Sygnatury zaktualizowane." if ok else "Nie udało się zaktualizować sygnatur. Sprawdź logi i freshclam.")
        self.refresh_all()

    def refresh_all(self):
        state = self.agent.get_state()
        self.status_label.config(text=state["status"], fg=OK if state["status"] == "ACTIVE" else WARN)
        self.av_label.config(text="CLAMAV READY" if state["clamav_ready"] else "CLAMAV OFFLINE", fg=OK if state["clamav_ready"] else WARN)
