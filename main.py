...
class DashboardView(tk.Frame):
    def __init__(self, parent, agent, run_cb, stop_cb, scan_cb, reload_ml_cb):
        super().__init__(parent, bg=BG)
        self.agent = agent
        self.run_cb = run_cb
        self.stop_cb = stop_cb
        self.scan_cb = scan_cb
        self.reload_ml_cb = reload_ml_cb
        self.cards = {}
        self._build()
    ...
    def _build(self):
        ...
        tk.Label(
            self,
            text="Lokalny agent skanuje Downloads, procesy i połączenia, a model ONNX ocenia podobieństwo do wzorców zagrożeń.",
            bg=BG,
            fg="#d7ddff",
            font=("Segoe UI", 13)
        ...
        tk.Button(
            actions,
            text="Przeładuj model",
            command=self.reload_ml_cb,
            bg="#103040",
            fg="#d8fbff",
            relief="flat",
            bd=0,
        ...
        desc = (
            "Reguły klasyczne nadal wykrywają ryzykowne rozszerzenia, procesy i porty. "
            "Warstwa ONNX zamienia opis zdarzenia na embedding i porównuje go do wzorców zagrożeń "
            "zdefiniowanych w config.json. Wynik similarity trafia do alertów i logów."
        )
        tk.Label(
            info,
            text=desc,
    ...
    def refresh(self):
        ...
        self.cards["Stan"][1].config(text="ACTIVE = skany okresowe, IDLE = agent zatrzymany.")
        ...
        self.cards["Model ML"][0].config(
            text="READY" if state["model_ready"] else "OFFLINE"
        )
        self.cards["Model ML"][1].config(
            text=f'{state["model_name"]}\nthreshold={state["ml_threshold"]}'
        )
        ...
        self.cards["Procesy"][1].config(text="Dopasowania słów kluczowych w nazwach lub argumentach.")

        self.cards["Połączenia"][0].config(text=str(state["network_hits"]))
        self.cards["Połączenia"][1].config(text="Dopasowania połączeń na oznaczonych portach.")

        last_score = state["last_ml_score"]
        score_text = "-" if last_score is None else f"{last_score:.3f}"
        self.cards["Ostatni score"][0].config(text=score_text)
        self.cards["Ostatni score"][1].config(text=state["last_ml_pattern"] or "Brak ostatniego wzorca.")
...
class AlertsView(tk.Frame):
    ...
    def refresh(self):
        ...
        if not alerts:
            tk.Label(
                self.inner,
                text="Brak alertów. Uruchom skan lub pozwól agentowi pracować w tle.",
                bg=PANEL,
                fg="#c8cff9",
                font=("Segoe UI", 12)
...
class LogsView(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(self, text="Pełny dziennik lokalnego agenta ochrony i warstwy ML.", bg=BG, fg=MUTED, font=("Segoe UI", 11)).pack(anchor="w", padx=24)
...
class SettingsView(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(self, text="Tu zmieniasz folder Downloads, interwał skanu, wzorce zagrożeń i ustawienia modelu ONNX.", bg=BG, fg=MUTED, font=("Segoe UI", 11)).pack(anchor="w", padx=24)
...
class MusgGuardApp(tk.Tk):
    ...
    def _build_shell(self):
        ...
        self.model_label = tk.Label(sidebar, text="MODEL OFFLINE", fg=WARN, bg="#0d0b1a", font=("Segoe UI", 11, "bold"))
        self.model_label.pack(anchor="w", padx=24, pady=(0, 18))
        ...
        tk.Button(quick, text="Przeładuj model", command=self.reload_model, bg="#103040", fg="#d8fbff", relief="flat", bd=0, font=("Segoe UI", 12, "bold"), padx=18, pady=10).pack(fill="x", pady=(10, 0))

        self.main = tk.Frame(self, bg=BG)
        self.main.pack(side="left", fill="both", expand=True)

        self.views = {
            "dashboard": DashboardView(self.main, self.agent, self.run_agent, self.stop_agent, self.scan_now, self.reload_model),
    ...
    def refresh_all(self):
        ...
        self.model_label.config(
            text=f'MODEL {"READY" if state["model_ready"] else "OFFLINE"}',
            fg=OK if state["model_ready"] else WARN
        )
